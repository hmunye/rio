...
//! concurrently are __preemptive__ and __cooperative__.
//!
//! __Preemptive__ multitasking is the method in which the operating system (OS)
...
//! known as a __context switch__. The thread's stack, which is allocated
...
//! stack.
//!
//! __Cooperative__ multitasking, in contrast, delegates the responsibility of
//! yielding CPU time to tasks themselves. When combined with __asynchronous
//! programming__, cooperative multitasking allows tasks to continue executing
...
//! The downside of this approach is that a misbehaving task can __starve__
//! other tasks or prevent them from making progress all together.
//!
...
//! __runtime__ to manage task scheduling and polling. It is responsible for
